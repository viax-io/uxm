...
All 95 components exported from `@viax/uxm/ui` (as of v4.26.0), grouped by intent. Use this file
...
| **Checkbox** | `Checkbox`, `CheckboxProps` | Label-wrapped native input + custom box; controlled. ⚠️ **`onChange` is `(checked: boolean, e) => void`** — the boolean comes first, NOT a bare native event (so `e.target.checked` is wrong). Label content goes in `children`. `error?: string` (2.8.0) — sets `aria-invalid` + a FieldError message below; box/label stay neutral. (unreleased) Hover now paints in consumer apps: `--uxm-checkbox-hover-unchecked-border` falls back to `--color-text-muted` and `--uxm-checkbox-hover-checked-bg` / `-border` to `--color-accent-bold` (they used to echo the resting colours, so the `:hover` rule was dead outside the studio); hover is skipped while `.uxm-checkbox--disabled`, which now dims to `0.4` (was `0.6`). |
...
| **RadioGroup / RadioOption** | `RadioGroup`, `RadioOption`, `RadioGroupDirection`, `RadioGroupProps`, `RadioOptionProps` | Radio group with composable options. `direction: 'vertical' \| 'horizontal'`. `RadioGroup` takes `error?: string` (2.8.0) — `aria-invalid` + a group FieldError message below; circles/labels stay neutral. (unreleased) `--disabled` dims to `0.4` (was `0.6`) via `--uxm-radio-group-disabled-opacity`, and hover now paints in consumer apps: `--uxm-radio-group-hover-unselected-border` falls back to `--color-text-muted`, `-hover-selected-border` / `-hover-dot-color` to `--color-accent-bold` (they used to echo the resting colours, so the `:hover` rule was dead outside the studio); hover is skipped while `.uxm-radio--disabled`. |
...
| **SectionHeader** | `SectionHeader`, `SectionHeaderProps` | Uppercase heading; `children` (heading text), `trailing?: ReactNode` (right-aligned action/status), `subtitle?: ReactNode` (context line below). |
...
| **Icon** | `Icon`, `IconProps`, `ICONS`, `ICON_OPTIONS`, `getIcon`, `IconDef` | SVG by `glyph` id from the registry. Props: `glyph` (required), `size` (default **24**), `strokeWidth` (default **1.75**). Inherits `currentColor`. **Returns `null` for an unknown glyph — it fails silently, so only use ids from the registry below.** Decorative by default (unreleased): renders `aria-hidden="true"` unless you pass `aria-label`, which switches it to `role="img"` — icon+text buttons get clean accessible names; name icon-only controls on the control itself. |
...
| Switch context (workspace / scope / view) from a field-like trigger | `Menu` (unreleased) — `current` marks where you are, `matchAnchorWidth="min"` ties the panel to the trigger. **Not** `Select`/`Listbox`: switching is an action with side effects, and the panel usually hosts a command ("New workspace") that `role=listbox` can't contain |
| The rail row that opens that switcher (or the account menu) | `SidebarNavTrigger` (unreleased) — a `<button>` built for `triggerProps`; caption + value + chevron, no icon tile. **Not** `SidebarNavItem`: that one is an `<a>` that navigates |
