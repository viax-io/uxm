...
All 96 components exported from `@viax/uxm/ui` (as of v4.35.0), grouped by intent. Use this file
...
| **FileUpload** | `FileUpload`, `FileUploadFileMeta`, `FileUploadState`, `FileStatus`, `FileUploadProps` | Drop area + per-file progress rows; controlled via state prop. Page-level error via `error?: string` (2.8.0; renamed from `errorMessage`, which stays a deprecated alias); per-file errors via `FileUploadFileMeta.errorMessage`. *(unreleased)* A `done` row can offer the file back: `FileUploadFileMeta.href` renders `<a href download>`, or `onOpenFile?(id)` renders a `<button>` — use the callback when the URL needs an auth header (an anchor sends none), `href` otherwise for native new-tab/save-as. Both set → `onOpenFile` wins; neither → no control. `download` is ignored cross-origin (navigates instead of saving) — `downloadGlyph="arrow-up-right"` then. |
...
| Edit or view code / config / JSON / query text | `CodeEditor` (editable `<textarea>`, unreleased) or `CodeBlock` (read-only output, unreleased) — no syntax highlighting, by design |
