...
All 92 components exported from `@viax/uxm/ui` (as of v4.3.0), grouped by intent. Use this file
...
| **FormField** | `FormField`, `FormFieldLabelPosition`, `FormFieldProps` (unreleased: `FormFieldLabelTone`, `FormFieldLabelVariant`) | Label + optional help text wrapper. `labelPosition: 'top' \| 'side'`. **Owns label rendering** — atoms below render bare. Field errors live on the ATOM's `error` prop, not here. (unreleased) Two orthogonal label axes: `labelTone: 'strong' \| 'default' \| 'muted'` — colour, each tint themable via `--uxm-form-field-label-tint-*` (muted = detail panels where the value dominates); `labelVariant: 'default' \| 'overline'` — typography, overline = small caps "eyebrow" with its own `--uxm-form-field-overline-*` tokens (colour still comes from the tint). An `EditableCell` child is auto-outdented so its text sits on the label edge (its display state is a button — pair it with an explicit `aria-label`). |
...
| Detail card of labelled EDITABLE rows (entity name/status/…) | `Card gap` + `Stack` + `FormField labelPosition="side" labelTone="muted"` + `EditableCell` (unreleased) |
