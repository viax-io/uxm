...
All 97 components exported from `@viax.io/uxm/ui` (as of v4.48.0) — imported as `uxm/ui` through the `uxm` alias (SKILL.md → "Before writing any code"), grouped by intent. Use this file
...
|-----------|-------------|------------|
| **Calendar** | `Calendar`, `CalendarValue`, `CalendarProps` | Three-view (day/month/year) calendar, single + range selection, controlled or uncontrolled. "Today" footer button jumps to the current month; `shadow` boolean (default true — pass false for a flat embedded calendar). (4.37.0) `locale` now defaults to the nearest `UxmLocaleProvider` instead of a hardcoded `"en-US"` — month/weekday names follow the app; pass `locale` to pin one calendar. Button names stay English defaults (`previousMonthLabel` / `nextMonthLabel` / `drillUpLabel`). |
...
| **CurrencyInput** | `CurrencyInput`, `CurrencyValue`, `CurrencyInputProps` | Left-aligned amount + currency picker with searchable popover, locale formatting. `clearable?: boolean` (**default `true`**) — self-clearing trailing ✕, no `onClear` (3.0.0, which also dropped `pickerPosition`). (4.37.0) `locale` now defaults to the nearest `UxmLocaleProvider` instead of a hardcoded `"en-US"`. |
| **DateInput** | `DateInput`, `DateInputFormat`, `DateInputMode`, `DateInputProps` | Masked date field, single/range, formats `mdy`/`dmy`/`ymd`. Focusing the field opens the Calendar picker (type-or-pick). `clearable?: boolean` (**default `true`**) — self-clearing ✕ by the calendar icon, no `onClear` (3.0.0). (4.37.0) `invalidMessage?: string` replaces the built-in "Enter a valid date (MM/DD/YYYY)" raised on blur (a consumer `error` still wins over both); the typing mask itself is overridden with the existing `placeholder` prop. |
| **FileUpload** | `FileUpload`, `FileUploadFileMeta`, `FileUploadLabels`, `FileUploadState`, `FileStatus`, `FileUploadProps` | Drop area + per-file progress rows; controlled via state prop. Page-level error via `error?: string` (2.8.0; renamed from `errorMessage`, which stays a deprecated alias); per-file errors via `FileUploadFileMeta.errorMessage`. A `done` row can offer the file back: `FileUploadFileMeta.href` renders `<a href download>`, or `onOpenFile?(id)` renders a `<button>` — use the callback when the URL needs an auth header (an anchor sends none), `href` otherwise for native new-tab/save-as. Both set → `onOpenFile` wins; neither → no control. `download` is ignored cross-origin (navigates instead of saving) — `downloadGlyph="arrow-up-right"` then. (4.37.0) `labels?: FileUploadLabels` covers the copy the atom generates per row — `uploading`, `uploadFailed`, and the three composed ones as functions: `uploadProgress(percent, size)`, `removeFile(name)`, `cancelFile(name)` (functions, not prefixes — word order around a filename is language-specific). File sizes now format through `Intl` (`locale`, defaulting to the nearest `UxmLocaleProvider`), so the unit and decimal separator localise; ⚠️ both the unit AND the number shift in English — `Intl`'s `kilobyte` is SI (1000 B) where the old formatter divided by 1024, so the same 482,304-byte file reads `482.3 kB` instead of `471.0 KB`, and sub-1000 sizes read the pluralised `512 bytes` instead of `512 B` — pass `formatSize?: (bytes) => string` for a house style such as binary `KiB` (`Intl` has no binary unit). |
...
| **RadioGroup / RadioOption** | `RadioGroup`, `RadioOption`, `RadioGroupDirection`, `RadioGroupProps`, `RadioOptionProps` | Radio group with composable options. `direction: 'vertical' \| 'horizontal'`. `RadioGroup` takes `error?: string` (2.8.0) — `aria-invalid` + a group FieldError message below; circles/labels stay neutral. `--disabled` dims to `0.4` (was `0.6`) via `--uxm-radio-group-disabled-opacity`, and hover now paints in consumer apps: `--uxm-radio-group-hover-unselected-border` falls back to `--color-text-strong`, `-hover-selected-border` / `-hover-dot-color` to `--color-accent-bold` (they used to echo the resting colours, so the `:hover` rule was dead outside the studio); hover is skipped while `.uxm-radio--disabled`. **(unreleased)** `variant='default' \| 'card'` + `indicator='hidden' \| 'corner'` — `card` turns each option into a selectable tile (children fill the body, selected = accent frame, circle hidden or cornered) for a visual picker, keeping the native radio semantics a hand-wired `Card` grid throws away. |
| **RangeSlider** | `RangeSlider`, `RangeSliderProps` | Dual-thumb range; shares slider styling. (4.37.0) `startLabel` / `endLabel` name the two thumbs verbatim — the default still composes `` `${aria-label} (start)` ``, which only reads correctly in English. |
...
| Component | Key exports | What it is |
|-----------|-------------|------------|
| **ButtonPrimary / Secondary / Tertiary / Ghost / Danger** | `ButtonPrimary`, `ButtonSecondary`, `ButtonTertiary`, `ButtonGhost`, `ButtonDanger`, `ButtonProps` | Five button variants on native `<button>`. `type` defaults to `'button'`. **(unreleased)** all five take a `ref` (`ButtonProps` is `ComponentPropsWithRef<'button'>`, placed on the `<button>`) — so a button can be a `HoverTooltip` / `Popover` trigger directly, with no wrapper span. **`ButtonDanger`** (2.6.0) is the outlined destructive variant for irreversible actions (Delete/Remove/Discard) — danger token trio, solid danger fill when pressed. From `./button`. |
...
| **MenuButton** **(unreleased)** | `MenuButton`, `MenuButtonProps`, `MenuButtonVariant` | The labelled dropdown button — `Menu` + a shipped `Button*` trigger with the chevron, ARIA and trigger ref already wired. `items` (same `MenuEntry[]`), `children` (the label), `variant: 'primary' \| 'secondary' \| 'tertiary' \| 'ghost'` (default `secondary`), `icon` (leading), `chevron` (default true, rotates while open), `placement`, `disabled`, `aria-label` (names the PANEL), `className` (on the trigger). Reach for it instead of hand-writing `renderTrigger` + a manual `<Icon glyph="chevron-down">`; drop back to `Menu` + `renderTrigger` for a bespoke trigger, controlled `open`, `matchAnchorWidth`, or per-instance panel styling — none of which it exposes. A composition, so it is themed entirely by `Button*` + `Menu` — including the icon/label/chevron gap, which stays the button's own `--uxm-button-{variant}-gap` (8px); it adds only the chevron rotation (honours `prefers-reduced-motion`). From `./menu`. |
...
| Component | Key exports | What it is |
|-----------|-------------|------------|
| **AppSidebar** | `AppSidebar`, `AppSidebarBrand`, `AppSidebarSection`, `AppSidebarNavItem`, `AppSidebarProps`, `AppSidebarIconColor`, `DEFAULT_SIDEBAR_ICON_COLORS` | App-shell left sidebar. `brand: { logoUrl?, iconUrl?, alt? }` (⚠️ no `name` prop), `sections: { heading?, items }[]` (⚠️ the section key is **`heading`**, not `label`), plus `collapsed`, `onCollapseToggle`, `header`, `footer`, `linkAs`, `mobileOpen`, `onMobileClose`. **The mobile drawer CSS ships (unreleased)** — below 768px the rail goes `position: fixed` and closed, `mobileOpen` slides it in over a scrim at `--z-drawer`; wire `mobileOpen`/`onMobileClose` and it works, no consumer media queries. (Before this it rendered the classes with no CSS: the rail stayed in flow and pushed the page sideways.)  Items are `{ href, label, icon?, active?, iconBg?, iconColor?, badge?, trailing? }` — `badge` is a persistent after-label marker (shows at rest, no hover gate, unlike `trailing`); when collapsed it stands in as a corner status dot on the icon tile, while the label + badge text stay in the accessibility tree (visually clipped) so the row announces identically in both states. `autoIconColors?: boolean \| AppSidebarIconColor[]` auto-assigns a distinct icon colour per item from a palette, cycling by position (unbroken across sections). Setting **either** `iconColor` or `iconBg` on an item opts it out of auto **entirely** — the unset half falls to the component default, not the palette. **`linkAs` receives `href`** — fine for `next/link`, but react-router's `Link` needs `to`, so wrap it in a `forwardRef` adapter (see quick-recipes §1) or nav breaks/reloads. `header?: ReactNode` is a slot at the **head** of the rail (DOM `uxm-app-sidebar__lead` — `__header` is the brand row) — that is where a tenant/workspace switcher belongs, since a control naming the level everything below it sits in has to be above it; the footer was previously the only slot, inverting the hierarchy. **`footer` now renders when collapsed** (it used to be dropped, taking the account row out of the rail) — a version caption there will now show in the 64px rail, so swap it on `collapsed`. Neither slot adapts its child: pass `collapsed` down and size the marks yourself (`Avatar size="small"`). Both slots are flex columns with an 8px gap (so two stacked switchers space themselves; `--uxm-app-sidebar-{lead,footer}-gap`) and use the nav's 8px padding so a control aligns with the nav rows (`--uxm-app-sidebar-{lead,footer}-padding`) — the footer's old 24px caption inset put a control 16px further in and ellipsised a signed-in address. |
...
| **List / ListItem** | `List`, `ListItem`, `ListProps`, `ListItemProps` | Vertical list container + item. ListItem polymorphs to div/button/anchor based on `interactive` / `href`. Two mutually exclusive leading slots: `icon` (wrapped in an accent `IconTile`, so a picture passed here gets tiled — not what you want) and **`media` (unreleased)**, which renders the node as-is for a `Thumbnail` / `Avatar` / `<img>` in a square `--uxm-list-item-media-size` box (36px, forwarded into `--uxm-thumbnail-size` / `--uxm-avatar-size` so those atoms scale with the row); `media` wins if both are passed. ⚠️ `media` lands inside the row's `<button>`/`<a>` — pass `alt=""` unless the image says something the row's text doesn't. `value` is a `ReactNode`, so a two-line sub-label needs no extra prop. Use `interactive` + `media` for product / people pickers instead of hand-wiring a `<button>` row. `List` takes **`variant='card' \| 'plain'` (unreleased)** — `plain` drops background/border/radius/clipping for a list already inside a surface; use it instead of overriding `.uxm-list` from your own CSS. **(unreleased)** `selected` / `onSelectedChange` add a **selectable** mode: the row becomes a `<label>` around a real checkbox, so a click anywhere toggles it — the multi-select shape (`active`+`aria-pressed` is a toggle BUTTON, wrong for a member of a checkable set). Mutually exclusive with `interactive`/`href` (selectable wins + dev warning); composes with `media`/`value`/`trailing`, leading order checkbox → media → content → trailing. |
...
- **Copy is the consumer's**, passed in as props. Every user-visible string an atom can render has
  a prop with an English default — `clearLabel`, `closeLabel`, `emptyState`, `countLabel`,
  `requiredMessage`, `placeholder`, `labels: {…}`. Translate in your app, pass the result down.
- **Formatting is the library's**, driven by one locale. Month names, decimal separators, and byte
  units come from `Intl` and can't be expressed as a prop string, so they read a locale instead.

| Component | Key exports | What it is |
|-----------|-------------|------------|
| **LanguageSwitcher** | `LanguageSwitcher`, `LanguageSwitcherProps`, `LanguageSwitcherVariant` | The single library-owned language control — globe + current endonym + caret over the shared Listbox panel. **Presentational and data-free**: takes `locales: string[]` (BCP-47, e.g. a realm's `getSupportedLocales`), `value`, `onChange`; it never queries, reads no context, persists nothing. **Renders `null` when `locales.length <= 1`** — not hidden, not disabled, nothing emitted, so a single-language realm pays zero layout cost. Labels are **endonyms** from `Intl.DisplayNames` (`Deutsch`, not `German`), resolved on the base subtag so `de-DE` reads `Deutsch` rather than `Deutsch (Deutschland)`; the region is re-added only where two tags share a base language (`en-US` + `en-GB` → `American English` / `British English`). `variant: 'full' \| 'compact'` (compact = icon-only, ≥34px target). `label` (default `"Language"`) is the trigger's accessible name and the ONLY thing naming the control — translate it. Rows carry `lang={locale}` so AT switches voice. Panel inherits `--uxm-listbox-*`; only the trigger has its own `--uxm-language-switcher-*` surface. |
| **UxmLocaleProvider** | `UxmLocaleProvider`, `UxmLocaleProviderProps`, `useUxmLocale`, `DEFAULT_UXM_LOCALE` | Context that broadcasts one BCP-47 tag to every atom formatting through `Intl` — `Calendar`, `DateInput` and `EditableCell` (via Calendar), `CurrencyInput`, `FileUpload`. Mount once at the app root: `<UxmLocaleProvider locale="uk-UA">`. An atom's own `locale` prop still wins, so one always-USD amount can opt out. No state, no browser API — SSR-safe; nesting overrides per subtree. `useUxmLocale(explicit?)` resolves prop → provider → `DEFAULT_UXM_LOCALE` (`'en-US'`), and is what an atom calls internally. It does **not** translate copy: a Ukrainian calendar whose prev/next buttons still announce "Previous"/"Next" is working as designed — pass `previousMonthLabel` / `nextMonthLabel`. |
...
<UxmLocaleProvider locale="uk-UA">
  <DateInput
    ...
    openCalendarLabel={t('date.open')}
    calendarDialogLabel={t('date.choose')}
    invalidMessage={t('date.invalid')}
  />
  <FileUpload
    titleText={t('upload.title')}
    helpText={t('upload.help')}
    labels={{
      ...
      cancelFile: (name) => t('upload.cancel', { name }),
    }}
  />
</UxmLocaleProvider>
```

⚠️ **Composed names take a function, not a prefix.** `removeFile(name)` / `cancelFile(name)` /
`uploadProgress(percent, size)` / `BulkActionBar`'s `countLabel(count)` are callbacks precisely
because word order and pluralisation around an interpolated value differ by language — a
`"Remove" + name` concatenation cannot express that. Never rebuild them by string-joining a
translated prefix.
...
| Click-to-select row with a thumbnail (product / people picker) | `List` + `ListItem interactive media={<Thumbnail/>} active` (unreleased) — not a hand-wired `<button>` |
...
| Localise dates / amounts / file sizes app-wide | `UxmLocaleProvider` at the root — formatting only; translated copy still goes in via each atom's label props |
| Let the user change the app's language | `LanguageSwitcher` — pass the locale list in; it fetches nothing and disappears entirely when only one locale is configured |
