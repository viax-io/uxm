...
All 97 components exported from `@viax.io/uxm/ui` (as of v4.48.2) — imported as `uxm/ui` through the `uxm` alias (SKILL.md → "Before writing any code"), grouped by intent. Use this file
...
| **RadioGroup / RadioOption** | `RadioGroup`, `RadioOption`, `RadioGroupDirection`, `RadioGroupProps`, `RadioOptionProps` | Radio group with composable options. `direction: 'vertical' \| 'horizontal'`. `RadioGroup` takes `error?: string` (2.8.0) — `aria-invalid` + a group FieldError message below; circles/labels stay neutral. `--disabled` dims to `0.4` (was `0.6`) via `--uxm-radio-group-disabled-opacity`, and hover now paints in consumer apps: `--uxm-radio-group-hover-unselected-border` falls back to `--color-text-strong`, `-hover-selected-border` / `-hover-dot-color` to `--color-accent-bold` (they used to echo the resting colours, so the `:hover` rule was dead outside the studio); hover is skipped while `.uxm-radio--disabled`. `variant='default' \| 'card'` + `indicator='hidden' \| 'corner'` — `card` turns each option into a selectable tile (children fill the body, selected = accent frame, circle hidden or cornered) for a visual picker, keeping the native radio semantics a hand-wired `Card` grid throws away. |
...
| Click-to-select row with a thumbnail (product / people picker) | `List` + `ListItem interactive media={<Thumbnail/>} active` — not a hand-wired `<button>` |
