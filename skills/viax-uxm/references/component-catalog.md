...
All 95 components exported from `@viax/uxm/ui` (as of v4.27.0), grouped by intent. Use this file
...
| **RadioGroup / RadioOption** | `RadioGroup`, `RadioOption`, `RadioGroupDirection`, `RadioGroupProps`, `RadioOptionProps` | Radio group with composable options. `direction: 'vertical' \| 'horizontal'`. `RadioGroup` takes `error?: string` (2.8.0) — `aria-invalid` + a group FieldError message below; circles/labels stay neutral. (unreleased) `--disabled` dims to `0.4` (was `0.6`) via `--uxm-radio-group-disabled-opacity`, and hover now paints in consumer apps: `--uxm-radio-group-hover-unselected-border` falls back to `--color-text-strong`, `-hover-selected-border` / `-hover-dot-color` to `--color-accent-bold` (they used to echo the resting colours, so the `:hover` rule was dead outside the studio); hover is skipped while `.uxm-radio--disabled`. |
...
| **ErrorPage** | `ErrorPage`, `ErrorPageProps` | Full-page error layout. Code + title follow the brand heading font/weight (`--uxm-error-page-code-font` / `-code-weight`, `-title-font` / `-title-weight`) *(unreleased)*. Props: `code`, `icon`, `title` (required), `message`, `primaryAction`, `secondaryAction`. ⚠️ Not `description` / `action` — those are `EmptyState`'s names. |
...
| **DetailSection** | `DetailSection`, `DetailSectionProps` | Title + icon-tile + content section. Title follows the brand heading font/weight (`--uxm-detail-section-title-font` / `-title-weight`) *(unreleased)*. |
...
| **PageHeader** | `PageHeader`, `PageHeaderProps` | Icon-tile + title + meta + actions row. Title follows the brand heading font/weight (`--uxm-page-header-title-font` / `-title-weight`) *(unreleased)*. ⚠️ **`meta` renders inside a `<p>`** — pass inline content only (string/`<span>`/fragment); a block-level primitive like `Stack` there is invalid `<div>`-in-`<p>` nesting and triggers a hydration warning. |
...
| **StatCard** | `StatCard`, `StatCardTrend`, `StatCardProps` | Label + headline value + trend chip (up/down/neutral). The value follows the brand heading font/weight (`--uxm-stat-card-value-font` / `-value-weight`, default 700) *(unreleased)*. |
...
| **SegmentRow** | `SegmentRow`, `SegmentRowProps` | Segment header — drag · accent line · expand chevron · name · count badge. Title follows the brand heading font/weight (`--uxm-segment-row-title-font`) *(unreleased)*. Building block. |
