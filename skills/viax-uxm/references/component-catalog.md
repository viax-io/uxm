...
All 95 components exported from `@viax/uxm/ui` (as of v4.33.0), grouped by intent. Use this file
...
| **CodeEditor / CodeBlock** | `CodeEditor`, `CodeBlock`, `CodeEditorProps`, `CodeBlockProps` | *(unreleased)* Developer-tooling code surface. `CodeEditor` = a real `<textarea>`: mono, spellcheck off, `Tab`/`Shift+Tab` indent+outdent (selection-aware), `Enter` auto-indent, `lineNumbers`, `wrap`, `indentSize`, `indentWithTab`, `autoIndent`, `error`, `textareaRef`. Escape-then-Tab leaves the field, so it is not a keyboard trap; `onKeyDown` fires before the atom's own handling (`preventDefault()` claims a key, e.g. ⌘Enter to run). `CodeBlock` = the read-only `<pre><code>` half for output/SDL/payload dumps, same gutter + `wrap`. **No syntax highlighting** — impossible inside a `<textarea>`, and out of scope by design (zero runtime deps). Both share ONE `--uxm-code-editor-*` var set; `CodeBlock` has no registry entry so the output can't drift from the editor. Replaces hand-rolled mono `<Textarea>`. From `./code-editor`. |
