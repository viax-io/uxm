...
All 97 components exported from `@viax.io/uxm/ui` (as of v4.43.2) — imported as `uxm/ui` through the `uxm` alias (SKILL.md → "Before writing any code"), grouped by intent. Use this file
...
| **LifecycleEdgeLabel** | `LifecycleEdgeLabel`, `LifecycleEdgeLabelProps`, `LifecycleEdgeLabelVariant` | Midpoint pill: `true` / `false` / `neutral`. **(unreleased)** a fourth variant `accent` for hierarchy (entry pill / model name) — AA in both themes, and its border rather than its fill is what separates it from the canvas. ⚠️ Static text ONLY: a plain `<span>`, no hover/focus/pressed — a clickable edge label is a `Chip`. |
