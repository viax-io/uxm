...
| Component | Key exports | What it is |
|-----------|-------------|------------|
| **AppSidebar** | `AppSidebar`, `AppSidebarBrand`, `AppSidebarSection`, `AppSidebarNavItem`, `AppSidebarProps`, `AppSidebarIconColor`, `DEFAULT_SIDEBAR_ICON_COLORS` | App-shell left sidebar. `brand: { logoUrl?, iconUrl?, alt? }` (⚠️ no `name` prop), `sections: { heading?, items }[]` (⚠️ the section key is **`heading`**, not `label`), plus `collapsed`, `onCollapseToggle`, `footer`, `linkAs`, `mobileOpen`, `onMobileClose`. Items are `{ href, label, icon?, active?, iconBg?, iconColor?, badge?, trailing? }` — (unreleased) `badge` is a persistent after-label marker that stays visible even when collapsed (unlike `trailing`). `autoIconColors?: boolean \| AppSidebarIconColor[]` auto-assigns a distinct icon colour per item from a palette, cycling by position (unbroken across sections). Setting **either** `iconColor` or `iconBg` on an item opts it out of auto **entirely** — the unset half falls to the component default, not the palette. **`linkAs` receives `href`** — fine for `next/link`, but react-router's `Link` needs `to`, so wrap it in a `forwardRef` adapter (see quick-recipes §1) or nav breaks/reloads. |
...
| **SidebarNavItem** | `SidebarNavItem`, `SidebarNavItemProps` | Anchor-based nav row; polymorphic `as` prop. Two after-label slots: (unreleased) `badge` (persistent status marker — always visible, incl. collapsed) and `trailing` (hover-reveal, e.g. a remove ×); `badge` renders before `trailing`. |
