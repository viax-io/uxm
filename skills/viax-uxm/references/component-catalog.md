...
All 95 components exported from `@viax/uxm/ui` (as of v4.31.0), grouped by intent. Use this file
...
| **FormField** | `FormField`, `FormFieldLabelPosition`, `FormFieldProps`, `FormFieldLabelTint`, `FormFieldLabelVariant` | Label + optional help text wrapper (help text via **`hint?: ReactNode`**, rendered below the input and ANNOUNCED: FormField injects `aria-describedby` into its single child, merged with the child's own — so the control must be FormField's DIRECT child; a wrapping grid/row swallows the association. Input-family atoms merge, group-style atoms (RadioGroup, ToggleSwitch, SearchDropdown, NumberStepper) drop it). `labelPosition: 'top' \| 'side'`. **Owns label rendering** — atoms below render bare. Field errors live on the ATOM's `error` prop, not here. Two orthogonal label axes: `labelTint: 'strong' \| 'default' \| 'muted'` — colour, each tint themable via `--uxm-form-field-label-tint-*` (muted = detail panels where the value dominates); `labelVariant: 'default' \| 'overline'` — typography, overline = small caps "eyebrow" with its own `--uxm-form-field-overline-*` tokens (colour still comes from the tint). An `EditableCell` child is auto-outdented so its text sits on the label edge (its display state is a button — pair it with an explicit `aria-label`). `--uxm-form-field-side-label-valign` (default `center`; `start`/`end`) sets the label's vertical alignment in `side` layout — use `start` to top-align a label next to a tall control (radio/checkbox list). |
...
| **SectionHeader** | `SectionHeader`, `SectionHeaderProps` | Uppercase heading; `children` (heading text), `trailing?: ReactNode` (right-aligned action/status), `subtitle?: ReactNode` (context line below). `level?: 2–6` sets the heading tag (default 4); styling is unchanged *(unreleased)*. |
