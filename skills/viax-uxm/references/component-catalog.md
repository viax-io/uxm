...
All 97 components exported from `@viax/uxm/ui` (as of v4.37.1), grouped by intent. Use this file
...
| **RangeSlider** | `RangeSlider`, `RangeSliderProps` | Dual-thumb range; shares slider styling. (unreleased) `startLabel` / `endLabel` name the two thumbs verbatim — the default still composes `` `${aria-label} (start)` ``, which only reads correctly in English. |
...
| Localise dates / amounts / file sizes app-wide | `UxmLocaleProvider` (unreleased) at the root — formatting only; translated copy still goes in via each atom's label props |
| Let the user change the app's language | `LanguageSwitcher` (unreleased) — pass the locale list in; it fetches nothing and disappears entirely when only one locale is configured |
