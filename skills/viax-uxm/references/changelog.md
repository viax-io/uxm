...
at every release the pipeline (`scripts/stamp-skill-version.mjs`) promotes `### Unreleased` to
`### New in X.Y.Z` and rolls the oldest surplus section to the END of this file. Never hand-edit
the version markers; content edits are fine.

Read this file when a consumer is pinned below the oldest version still listed in `SKILL.md`, or
when a name in existing code is not in `references/component-catalog.md` (it was probably
renamed — the breaking notes for 3.0.0 and 4.0.0 are below).
...
- **`uxm/studio` ships the live style editor as a mountable component (`UxmApp`)** — the MODO
...
- **`fontFileUrl` / `safeFontFamily` are now exported from `uxm/studio/generate-css`** —
...
- **`required` + `requiredMessage` across the pickers** — `Select` (both modes), `PillSelect`,
  ...
  `uxm/ui` so every picker reports the identical text. Semantics differ by family, on purpose:
...
- **`AppSidebar` gained `autoIconColors`** (`boolean | AppSidebarIconColor[]`, default `false`).
  ...
  `DEFAULT_SIDEBAR_ICON_COLORS` (four DS hue pairs — warm / cool / accent / indigo — each clearing
  3:1 non-text contrast in BOTH themes); pass an array of `{ bg, color }` for your own palette.
  Setting **either** `iconColor` or `iconBg` on an item opts it out of auto **entirely** — the
  half you left unset falls to the component default, NOT to the palette — so a pinned colour is
  never spliced onto an auto half picked for a different tile. One item can pin its colour while
  the rest auto-fill. If you supply a custom palette, measure both themes: tile and ink are
  separate tokens that flip independently. New exports:
