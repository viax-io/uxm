...
- **The drawer CSS ships.** Below 768px — the same line `AppTopBar` uses to show its hamburger — the rail leaves the flow (`position: fixed`, full height, `--z-drawer`) and sits closed at `translateX(-100%)`; `--mobile-open` slides it in and the backdrop covers the viewport just beneath it. At `min-width: 768px` the backdrop is hidden and the rail is in flow as before, so a consumer holding `mobileOpen` true across a resize is not left with a scrim over its page. Consumers need no responsive rules of their own; wiring `mobileOpen` / `onMobileClose` is enough. The slide honours `prefers-reduced-motion: reduce`.
