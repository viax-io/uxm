...
| `autoIconColors` | `boolean \| AppSidebarIconColor[]` | `false` | Auto-assign a distinct icon colour to each item from a palette, **cycling by position** — a categorical scheme for scannability instead of a wall of same-coloured icons. `true` uses the exported `DEFAULT_SIDEBAR_ICON_COLORS` (four contrast-checked DS hue pairs); an array supplies your own `{ bg, color }`. An item that sets either `iconColor` or `iconBg` opts out of auto entirely (the unset half falls to the component default), so a pinned colour is never paired with an auto half chosen for a different tile — one item can pin its colour while the rest auto-fill. The cycle runs unbroken across section boundaries. |
