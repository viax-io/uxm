...
| `--uxm-stat-card-value-font` | `--type-display-font` → `--brand-heading-font` | `inherit` | Value typeface (**display** role). |
| `--uxm-stat-card-value-weight` | `--type-display-weight` → `--brand-heading-weight` | `700` | Value weight (**display** role). |
