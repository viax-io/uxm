...
| `hint` | `ReactNode` | – | Optional helper text shown below the control. **Associated with the control** via `aria-describedby`, so assistive tech reads it — merged with, not replacing, any `aria-describedby` the control sets itself (e.g. an input's error message; the whole input family merges via the shared `mergeDescribedBy` helper). **The control must be FormField's direct child**: the id is injected into the single child element, so a wrapping layout component (a grid or button row) swallows the association and the hint goes unannounced — put `style={{ alignSelf: 'start' }}` on the control instead of wrapping it, or skip `hint` on a composite and label the group itself. Group-style atoms that don't forward `aria-describedby` (`RadioGroup`, `ToggleSwitch`, `SearchDropdown`, `NumberStepper`) silently drop the association — give those a visible-text hint or their own described-by wiring. In `side` layout it is grid-placed in the control column. |
