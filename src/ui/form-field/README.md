...
| `hint` | `ReactNode` | – | Optional helper text shown below the control. **Associated with the control** via `aria-describedby`, so assistive tech reads it — merged with, not replacing, any `aria-describedby` the control sets itself (e.g. an input's error message). In `side` layout it is grid-placed in the control column. |
