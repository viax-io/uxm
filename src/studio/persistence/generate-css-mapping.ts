/**
 * Knob → CSS mapping tables for the overrides generator. Pure data: which
 * selector each component's rule is emitted on, and which knobs write a
 * custom property other than the default `--uxm-<id>-<kebab-key>`.
 *
 * Kept apart from the generator so the generator reads in one screen and so
 * `scripts/check-state-var-drift.mjs` (which parses this file for the
 * mapping) has one place to look. Every entry carries the reasoning for its
 * exception — keep it that way; the exceptions are the whole point.
 */
...
export const PER_COMPONENT_SELECTOR: Record<string, string> = {
...
export const PER_COMPONENT_MAPPING: Record<string, Record<string, string>> = {
  'input-with-icon': {
