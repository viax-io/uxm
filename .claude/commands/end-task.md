...
npm run check:drift   # registry-vs-SCSS state-var gate + type-scale + tokens parity
npm test              # Vitest smoke suite (tests/)
...
- `npm test` must be green. A change to keyboard/focus/ARIA behaviour, the CSS
  sanitizers or the overrides generator should come with a test in `tests/`;
  visual atoms do not.
